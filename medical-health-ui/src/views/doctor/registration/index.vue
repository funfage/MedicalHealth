<template>
  <div class="app-container">
      ...
      <el-row>
        ...
        <el-col :span="12">
          <div style="float:right">
            <span style="margin-right:10px">挂号费:￥{{ queryDeptParams.regItemAmount }}</span>
    ...
    <el-card>
      <!-- 挂号信息查询开始 -->
      <el-form ref="queryDeptForm" :model="queryDeptParams" :rules="regRules" :inline="true" label-width="83px">
...
export default {
  data() {
    return {
      ...
      regRules: {
        schedulingType: [
          { required: true, message: '挂号类型', trigger: 'blur' }
        ],
        subsectionType: [
          { required: true, message: '挂号时段', trigger: 'blur' }
        ],
        schedulingDay: [
          { required: true, message: '挂号时间', trigger: 'blur' }
        ]
      },
      // 部门数据
      deptTableList: []
  ...
  methods: {
    ...
    handleRegistration() {
      ...
      tx.$refs['form'].validate(valid => {
        if (valid) {
          tx.$refs['queryDeptForm'].validate(rValid => {
            if (rValid) {
              console.log(rValid)
