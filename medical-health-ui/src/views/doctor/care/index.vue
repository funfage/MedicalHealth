        ...
        <el-card>
          ...
          <el-tabs :stretch="true" type="card">
            <el-tab-pane label="患者档案">
              ...
              <div v-else style="text-align:center">暂无患者档案信息</div>
            </el-tab-pane>
          ...
          <el-tabs v-model="careActiveName" type="card">
            ...
            <el-tab-pane label="处方" name="c2">
              ...
              <div style="margin:3px;text-align:left">
                <el-button type="success" :disabled="careHistory.regId===undefined" icon="el-icon-plus" @click="handelAddMedicinesOrder">添加药用处方</el-button>
                <el-button type="success" :disabled="careHistory.regId===undefined" icon="el-icon-plus" @click="handelAddCheckItemOrder">添加检查处方</el-button>
              </div>
            </el-tab-pane>
          </el-tabs>
        </el-card>
...
import { queryToBeSeenRegistration, queryVisitingRegistration, queryVisitCompletedRegistration,
  ...
  getCareHistoryByRegId, queryCareOrdersByChId,
...
export default {
  ...
  methods: {
    ...
    viewRegistration() {
      ...
      this.getToBeSeenRegistration()
    },
    // 获取待就诊数据
    getToBeSeenRegistration() {
    ...
    handleRegistrationTabClick(tab, event) {
      if (tab.name === 't1') {
        ...
        this.getVisitingRegistration()
      } else if (tab.name === 't3') {
        this.getVisitCompletedRegistration()
      }
    },
    // 接诊
    handleVisit(row) {
    ...
    handleCareOrderItemDeleteByItemId(row) {
      ...
      const tx = this
      tx.$confirm('是否确定删除【' + itemName + '】这条详情数据，是否继续？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteCareOrderItemById(itemId).then(res => {
          tx.msgSuccess('删除成功')
          // 刷新处方数据
          tx.getCareOrdersByChId(this.careHistory.chId)
        }).catch(() => {
          tx.msgError('删除失败')
          tx.loading = false
        })
    ...
    handleCareOrderItemDelete(row) {
      ...
      this.submitCareOrder.careOrderItems.splice(index, 1)
    ...
    computeOrderItemAllAmount() {
      this.submitCareOrder.careOrder.allAmount = 0.0
